...
sampling=0.01/512;  % in meters 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  INITIALIZE HOLOGRAM FILM  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dimensions = 1; 
range = dimensions*0.01/2;
...
%objectpoints = [-0.00015 -0.00015 0.005;0.00015 0.00015 0.005];
...
%img = imread('./img/A-l1cm-p512','jpg');
%width = 0.01;  %generated image width
%height = 0.01; %generated image height
%depth=0.03;
...
%dz=objectpoints(1,3);
%dz=0.005
phase=2*pi*rand(size(ipx,2),size(ipy,2));
depth=0.07
dz=depth;
...
for o=1:n
    ...
    for i=1:size(ipx,2)
        for j=1:size(ipy,2)
            ...
            complexwave = exp(2*pi*sqrt(-1)*distance/(wavelength)+phase(i,j));
